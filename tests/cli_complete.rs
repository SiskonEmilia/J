...
fn complete_rich_sanitizes_newlines_and_tabs_in_values() {
  ...
  "roots": {
    ...
    c.env("J_CONFIG", &cfg)
        .args([":complete-rich", "powershell", &line.len().to_string(), line]);
    ...
    assert_eq!(records.len(), 1, "sanitization must collapse newlines in value; got:\n{}", out);
    let rec = records[0];
    assert_eq!(rec.chars().filter(|&c| c == '\t').count(), 1,
        "record must contain exactly one tab (field separator): {:?}", rec);
...
}}"#, root.display().to_string().replace('\\', "\\\\"))).unwrap();
